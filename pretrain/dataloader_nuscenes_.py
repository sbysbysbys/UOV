...
def minkunet_collate_pair_fn(list_data):
    ...
    return {
        ...
        "superpixels": superpixels_batch, # 好像就是图像？
    }
...
class NuScenesMatchDataset(Dataset):
    ...
    def map_pointcloud_to_image(self, data, min_dist: float = 1.0):
        ...
        superpoints_features = np.empty((0, self.feature_dim), dtype=np.int64)
        camera_list = [
            "CAM_FRONT",
            "CAM_FRONT_RIGHT",
            "CAM_BACK_RIGHT",
        ...
        _,_,lidarseg_pseudo_labels_filename = self.nusc.get("lidarseg", data["LIDAR_TOP"])["filename"].rpartition('/')
        lidarseg_pseudo_labels_path = os.path.join(
            self.lidarseg_ov_dir, lidarseg_pseudo_labels_filename
        ).replace(".bin", ".npy")
        lidarseg_features_path = os.path.join(
            self.lidarseg_ov_dir, lidarseg_pseudo_labels_filename
        ).replace(".bin", ".npz")
        points_pseudo_labels = np.load(lidarseg_pseudo_labels_path)
        superpoints_ids = np.floor(points_pseudo_labels/100)
        superpoints_labels = points_pseudo_labels%100
        features = np.load(lidarseg_features_path)
        ...
        for i, camera_name in enumerate(camera_list):
            ...
            mask = np.logical_and(mask, superpoints_ids[:] != -1)
        ...
        for key in features:
            superpoints_features = np.concatenate((superpoints_features, np.expand_dims(features[key], axis=0)))

        return pc_ref.T, images, pairing_points, pairing_images, np.stack(superpixels), superpoints_ids, superpoints_labels, superpoints_features
    ...
    def __getitem__(self, idx):
        ...
        superpoints_ids = torch.tensor(superpoints_ids[indexes])
        superpoints_labels = torch.tensor(superpoints_labels[indexes])
        superpoints_features = torch.tensor(superpoints_features)
