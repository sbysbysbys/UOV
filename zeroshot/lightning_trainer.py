...
from zeroshot.criterion import DownstreamLoss
...
class LightningDownstream(pl.LightningModule):
    def __init__(self, model, config):
        ...
        self.train_losses = []
        self.val_losses = []
        self.ignore_index = config["ignore_index"] # =0
    ...
    def configure_optimizers(self):
        ...
        if self._config.get("scheduler") and self._config["scheduler"] == 'steplr':
            ...
            scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer, int(.9 * self.num_epochs),
            )
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self._config["task"] == "zero-shot":
            loss = self.criterion(output_points, batch["pseudo_labels"])
        else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, lb in enumerate(batch["len_batch"]):
            ...
            # labels.append(batch["pseudo_evaluation_labels"][i])
            labels.append(batch["evaluation_labels"][i]) # gt label to validate
            offset += lb
