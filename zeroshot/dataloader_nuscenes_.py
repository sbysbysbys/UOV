...
def custom_collate_fn(list_data):
    ...
    labelized = len(input) == 9
    # evaluation_labels are per points, labels are per voxels
    if labelized:
        xyz, coords, feats, labels, evaluation_labels, pseudo_labels, pseudo_evaluation_labels, lidarseg_filename, inverse_indexes = input
    else:
    ...
    if labelized:
        ...
        return {
            ...
            "lidarseg_filename": lidarseg_filename,
        }
    else:
        return {
            "pc": xyz,
...
class NuScenesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.labels:
            points_pseudo_labels = torch.empty(0,0)
            unique_pseudo_labels = torch.empty(0,0)
            lidarseg_pseudo_labels_filename = ""
            if self.task == "zero-shot":
                ...
                points_pseudo_labels = np.load(lidarseg_pseudo_labels_path)%100
