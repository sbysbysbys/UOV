...
class FCCLIP(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            # here!!!
            # import pdb; pdb.set_trace()
            in_vocab_features = outputs["in_vocab_feature"]
            out_vocab_features = pooled_clip_feature
            ...
            processed_results = []
            for mask_cls_result, mask_pred_result, input_per_image, image_size, in_vocab_feature, out_vocab_feature in zip(
                mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes, in_vocab_features, out_vocab_features
            ):
                ...
                if self.panoptic_on:
                    panoptic_r = retry_if_cuda_oom(self.panoptic_inference)(mask_cls_result, mask_pred_result, in_vocab_feature, out_vocab_feature)
            ...
            processed_results[-1]["text_embedding"] = text_features
    ...
    def panoptic_inference(self, mask_cls, mask_pred, in_vocab_feature, out_vocab_feature):
        ...
        cur_in_vocab_feature = in_vocab_feature[keep]
        cur_out_vocab_feature = out_vocab_feature[keep]
        ...
        segments_info = []
        fin = torch.zeros(1,cur_in_vocab_feature.size()[-1]).to(cur_in_vocab_feature.device)
        fout = torch.zeros(1,cur_out_vocab_feature.size()[-1]).to(cur_out_vocab_feature.device)

        current_segment_id = 0

        if cur_masks.shape[0] == 0:
            # We didn't detect any mask :(
            return panoptic_seg, segments_info, fin, fout
        else:
            # take argmax
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if mask_area > 0 and original_area > 0 and mask.sum().item() > 0:
                    ...
                    fin = torch.cat([fin,cur_in_vocab_feature[k].unsqueeze(0)], dim = 0)
                    fout = torch.cat([fout,cur_out_vocab_feature[k].unsqueeze(0)], dim = 0)
