...
def compute_fcclip(cam, output_filename, text_embed_path):
    ...
    origin = os.path.join(nusc.dataroot, cam["filename"])
    origin_copy = output_filename.replace('.png', '_origin.png')
    shutil.copy(origin, origin_copy)
...
def get_parser():
    ...
    parser.add_argument(
        "--sp_folder",
        default="/root/wangfeiyue3new/sby/.data/fc-clip/superpixel_random",
    ...
    parser.add_argument(
        ...
        default=['MODEL.WEIGHTS', '/root/wangfeiyue3new/sby/fc-clip/fcclip_cocopan.pth'],
        # default=[],
...
if __name__ == "__main__":
    ...
    # for scene_idx in tqdm(range(segments[cuda_num-1], segments[cuda_num])):
