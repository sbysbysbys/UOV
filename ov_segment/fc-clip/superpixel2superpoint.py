...
def map_pointcloud_to_image(data, sp_root, args, min_dist: float = 1.0, if_save = False):
    ...
    for i, camera_name in enumerate(camera_list):
        ...
        rf = torch.load(rf_path)
        if args.method == 'fc-clip_in':
            rf = rf[0].numpy()
        if args.method == 'fc-clip_out':
            rf = rf[1].numpy()
        else:
            rf = rf.numpy()
    ...
    if if_save:
        ...
        # points_save_path = lidarseg_save_path.replace("_lidarseg.npy", "_points.npy")
        # np.save(points_save_path, pc_ref.T)
...
def parse_arguments():
    ...
    parser.add_argument('-l', '--lidarseg_save_folder', help='save lidarseg', type=str,
                        default='/root/wangfeiyue3new/sby/.data/fc-clip/lidarseg_ov/') 
    parser.add_argument('-m', '--method', help='superpixel_generate_method', type=str,
                        default='fc-clip_out') 
...
if __name__ == "__main__":
    ...
    for idx in tqdm(range(len(list_tokens))):
        ...
        if os.path.exists(lidarseg_save_path):
            all_check = 0
        if all_check:
            p_sameseg, p_labeled = map_pointcloud_to_image(lidar_token, args.sp_folder, args = args, if_save = False) # change here
            ps.append(p_sameseg)
            pl.append(p_labeled)
