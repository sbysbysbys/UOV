...
class SAN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            mask_emb = mask_embs[-1]
            text_emb = ov_classifier_weight[:-1]
            ...
            # import pdb; pdb.set_trace()
            processed_results = []
            for mask_cls_result, mask_pred_result, input_per_image, image_size, mask_emb_per_image in zip(
                mask_logits, mask_preds, batched_inputs, images.image_sizes, mask_emb
            ):
                ...
                #     mask_cls_result, mask_pred_result, mask_emb_per_image
                # )
                ...
                processed_results[-1]["image_emb"] = image_emb
                processed_results[-1]["text_emb"] = text_emb
                # pred = torch.einsum("hwd,cd->hwc", image_emb, text_emb).argmax(dim=-1)
                # predr = r.argmax(dim=0)
                # import pdb; pdb.set_trace()
    ...
    def semantic_inference(self, mask_cls, mask_pred, mask_emb):
